...
var VehicleTableColumns = struct {
	...
	UserDeviceID:  "vehicles.user_device_id",
	EncryptionKey: "vehicles.encryption_key",
}
...
var VehicleRels = struct {
	VehicleTokenTrips string
}{
	VehicleTokenTrips: "VehicleTokenTrips",
}

// vehicleR is where relationships are stored.
type vehicleR struct {
	VehicleTokenTrips TripSlice `boil:"VehicleTokenTrips" json:"VehicleTokenTrips" toml:"VehicleTokenTrips" yaml:"VehicleTokenTrips"`
}
...
func (r *vehicleR) GetVehicleTokenTrips() TripSlice {
	if r == nil {
		return nil
	}
	return r.VehicleTokenTrips
}
...
// VehicleTokenTrips retrieves all the trip's Trips with an executor via vehicle_token_id column.
func (o *Vehicle) VehicleTokenTrips(mods ...qm.QueryMod) tripQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"trips_api\".\"trips\".\"vehicle_token_id\"=?", o.TokenID),
	)

	return Trips(queryMods...)
}

// LoadVehicleTokenTrips allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (vehicleL) LoadVehicleTokenTrips(ctx context.Context, e boil.ContextExecutor, singular bool, maybeVehicle interface{}, mods queries.Applicator) error {
	var slice []*Vehicle
	var object *Vehicle
	...
	query := NewQuery(
		qm.From(`trips_api.trips`),
		qm.WhereIn(`trips_api.trips.vehicle_token_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if err != nil {
		return errors.Wrap(err, "failed to eager load trips")
	}

	var resultSlice []*Trip
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice trips")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on trips")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for trips")
	}
	...
	if singular {
		object.R.VehicleTokenTrips = resultSlice
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if queries.Equal(local.TokenID, foreign.VehicleTokenID) {
				local.R.VehicleTokenTrips = append(local.R.VehicleTokenTrips, foreign)
				if foreign.R == nil {
					foreign.R = &tripR{}
				}
...
// AddVehicleTokenTrips adds the given related objects to the existing relationships
// of the vehicle, optionally inserting them as new records.
// Appends related to o.R.VehicleTokenTrips.
// Sets related.R.VehicleToken appropriately.
func (o *Vehicle) AddVehicleTokenTrips(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Trip) error {
	var err error
	...
	if o.R == nil {
		o.R = &vehicleR{
			VehicleTokenTrips: related,
		}
	} else {
		o.R.VehicleTokenTrips = append(o.R.VehicleTokenTrips, related...)
	}
...
// SetVehicleTokenTrips removes all previously related items of the
// vehicle replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.VehicleToken's VehicleTokenTrips accordingly.
// Replaces o.R.VehicleTokenTrips with related.
// Sets related.R.VehicleToken's VehicleTokenTrips accordingly.
func (o *Vehicle) SetVehicleTokenTrips(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Trip) error {
	query := "update \"trips_api\".\"trips\" set \"vehicle_token_id\" = null where \"vehicle_token_id\" = $1"
	...
	if o.R != nil {
		for _, rel := range o.R.VehicleTokenTrips {
		...
		o.R.VehicleTokenTrips = nil
	}

	return o.AddVehicleTokenTrips(ctx, exec, insert, related...)
}

// RemoveVehicleTokenTrips relationships from objects passed in.
// Removes related items from R.VehicleTokenTrips (uses pointer comparison, removal does not keep order)
// Sets related.R.VehicleToken.
func (o *Vehicle) RemoveVehicleTokenTrips(ctx context.Context, exec boil.ContextExecutor, related ...*Trip) error {
	if len(related) == 0 {
		return nil
	}
	...
	for _, rel := range related {
		for i, ri := range o.R.VehicleTokenTrips {
			if rel != ri {
				continue
			}

			ln := len(o.R.VehicleTokenTrips)
			if ln > 1 && i < ln-1 {
				o.R.VehicleTokenTrips[i] = o.R.VehicleTokenTrips[ln-1]
			}
			o.R.VehicleTokenTrips = o.R.VehicleTokenTrips[:ln-1]
			break
		}
	}
...
func Vehicles(mods ...qm.QueryMod) vehicleQuery {
	mods = append(mods, qm.From("\"trips_api\".\"vehicles\""))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"\"trips_api\".\"vehicles\".*"})
	}
...
func FindVehicle(ctx context.Context, exec boil.ContextExecutor, tokenID types.Decimal, selectCols ...string) (*Vehicle, error) {
	vehicleObj := &Vehicle{}
	...
	query := fmt.Sprintf(
		"select %s from \"trips_api\".\"vehicles\" where \"token_id\"=$1", sel,
	)
...
func (o *Vehicle) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO \"trips_api\".\"vehicles\" (\"%s\") %%sVALUES (%s)%%s", strings.Join(wl, "\",\""), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO \"trips_api\".\"vehicles\" %sDEFAULT VALUES%s"
		}
...
func (o *Vehicle) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE \"trips_api\".\"vehicles\" SET %s WHERE %s",
...
func (o VehicleSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE \"trips_api\".\"vehicles\" SET %s WHERE %s",
...
func (o *Vehicle) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	...
	if !cached {
		...
		cache.query = buildUpsertQueryPostgres(dialect, "\"trips_api\".\"vehicles\"", updateOnConflict, ret, update, conflict, insert)
...
func (o *Vehicle) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"trips_api\".\"vehicles\" WHERE \"token_id\"=$1"
...
func (o VehicleSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"trips_api\".\"vehicles\" WHERE " +
...
func (o *Vehicle) Reload(ctx context.Context, exec boil.ContextExecutor) error {
	ret, err := FindVehicle(ctx, exec, o.TokenID)
	if err != nil {
		return err
	}
...
func (o *VehicleSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT \"trips_api\".\"vehicles\".* FROM \"trips_api\".\"vehicles\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, vehiclePrimaryKeyColumns, len(*o))
...
func VehicleExists(ctx context.Context, exec boil.ContextExecutor, tokenID types.Decimal) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"trips_api\".\"vehicles\" where \"token_id\"=$1 limit 1)"
...
func (o *Vehicle) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	return VehicleExists(ctx, exec, o.TokenID)
}
