...
func main() {
	...
	devicesConn, err := grpc.Dial(settings.DeviceGRPCAddr, grpc.WithTransportCredentials(insecure.NewCredentials()))
	if err != nil {
		logger.Fatal().Err(err).Msg("Failed to establish connection to devices-api.")
	}
	userDevicesClient := pb_devices.NewUserDeviceServiceClient(devicesConn)

	controller := consumer.New(esStore, bundlrClient, pgStore, userDevicesClient, &logger, settings.DataFetchEnabled, settings.WorkerCount, settings.BundlrEnabled)
